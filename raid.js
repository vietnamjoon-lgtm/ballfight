...
class RaidBattle extends Battle{
  constructor(config,entries,mode,random=Math.random){
    ...
    this.fighters=entries.map((entry,slot)=>{
      ...
      return {
        ...
        hp,maxHp:hp,radius,x:360,y:360,
        ...
        skillState:{},dash:null,shield:0,shieldTime:0,
        flash:0,trail:[],spin:0,facing:0
      };
    });
    this.api=Object.freeze({
      ...
      effect:(type,self,state,duration)=>{
        ...
        this.effects.push({
          ...copy(state),type,owner:self.slot,
  ...
  fits(f,placed){
    return f.x>=PAD+f.radius&&f.x<=SIZE-PAD-f.radius&&
      f.y>=PAD+f.radius&&f.y<=SIZE-PAD-f.radius&&
  ...
  place(){
    if(this.mode==='boss'){
      ...
      const low=boss.radius+big+16,high=348-big;
      ...
      for(let attempt=0;attempt<40;attempt++){
        ...
        for(const f of order){
          let found=false;
          for(let i=0;i<300;i++){
            const lo=PAD+f.radius+3,span=SIZE-2*lo;
            f.x=lo+this.random()*span;
            f.y=lo+this.random()*span;
            if(this.fits(f,placed)){found=true;break;}
          }
    ...
    if(this.state==='ready'){
      for(const f of this.fighters){
        for(const s of f.skills){
          if(this.types[s.type].trigger==='pickup')this.cast(f,s);
        }
      }
    }
  ...
  step(dt){
    ...
    for(const s of this.shots){
      ...
      if(owner.hp<=0){s.life=0;continue;}
      const x=s.x,y=s.y;
      s.x+=s.vx*dt;
      s.y+=s.vy*dt;
      s.life-=dt;
      ...
      const hits=this.enemies(owner).map(t=>{
        ...
        return {
          t,k,
          hit:Math.hypot(t.x-x-k*dx,t.y-y-k*dy)<=t.radius+s.radius
        };
      ...
      const outside=s.x<PAD+s.radius||s.x>SIZE-PAD-s.radius||
        s.y<PAD+s.radius||s.y>SIZE-PAD-s.radius;
      if(outside){
        if(s.bounces>0){this.wall(s);s.bounces--;}
        else s.life=0;
      }
    }
    ...
    for(const o of this.orbits){
      ...
      o.remaining-=dt;
      if(f.hp<=0){o.remaining=0;continue;}
      o.angle+=o.speed*dt;
      o.x=f.x+Math.cos(o.angle)*o.range;
      o.y=f.y+Math.sin(o.angle)*o.range;
      o.hits=o.hits||{};
      for(const t of this.enemies(f)){
        if(o.remaining>0&&(o.hits[t.slot]||0)<=this.time&&
           dist(o,t)<t.radius+9){
    ...
    if(bank){
      bank.renderer=this.effects.find(e=>
        e.type==='moneyMissile'&&this.fighters[e.owner].hp>0
      )?.owner;
    }
    for(const e of this.effects){
      const f=this.fighters[e.owner],target=this.effectTarget(e);
      if(f.hp<=0||!target){
        e.remaining=0;
        f.rooted=false;
        continue;
...
function mount(){
  ...
  function roster(){
    ...
    const data=raw?JSON.parse(raw):g.ArenaEngine.copy(g.ArenaDefaults);
    const add=(list,item)=>{
    ...
    if(g.ArenaPhonePack){
      g.ArenaPhonePack.abilities.forEach(a=>add(data.abilities,a));
  ...
  function buttons(){
    const busy=!!countdown||battle?.state==='running'||
      battle?.state==='paused';
  ...
  function preview(){
    try{
      const next=new g.RaidBattle(config,entries(),$('mode').value);
      sound.stopSamples();
      battle=next;countdown=null;acc=0;seenImpact=0;shake=0;
      ...
      for(const f of battle.fighters){
        ...
        card.style.cssText='padding:10px;border:1px solid #ccc;border-radius:8px';
        hp.append(card);
      }
      buttons();draw();
      return true;
  ...
  function row(id){
    ...
    host.style.cssText='display:flex;flex-wrap:wrap;align-items:center;gap:8px;margin:10px 0;padding:10px;border:1px solid #ccc';
    host.innerHTML='<select aria-label="참가 캐릭터"></select><label>크기 <input data-radius type="number" min="12" max="120" step="1" style="width:72px"></label><label>체력 <input data-hp type="number" min="10" max="100000" step="1" style="width:100px"></label><button type="button">빼기</button>';
    ...
    defaults();
    select.onchange=()=>{defaults();preview();};
  ...
  function draw(){
    ...
    ctx.save();
    if(!reduced&&shake>0)
      ctx.translate(Math.sin(last*.11)*shake*12,Math.cos(last*.13)*shake*12);
    ctx.beginPath();ctx.rect(6,6,708,708);ctx.clip();

    for(const r of battle.rings){
      ctx.globalAlpha=Math.max(0,r.life/.45);
      ctx.strokeStyle=r.color;ctx.lineWidth=3;
      ctx.beginPath();
      ctx.arc(r.x,r.y,r.from+(r.range-r.from)*(1-r.life/.45),0,Math.PI*2);
      ctx.stroke();
    }
    ctx.globalAlpha=1;
    ...
    for(const f of battle.fighters){
      ...
      if(img?.complete&&img.naturalWidth){
        ...
        ctx.beginPath();ctx.arc(0,0,f.radius,0,Math.PI*2);ctx.clip();
        ctx.drawImage(img,
          (img.naturalWidth-side)/2,(img.naturalHeight-side)/2,
          side,side,-f.radius,-f.radius,2*f.radius,2*f.radius);
        ctx.restore();
      }
      ...
      ctx.arc(f.x,f.y,f.radius+1,0,Math.PI*2);ctx.stroke();
      ctx.fillStyle='#333';ctx.font='bold 15px sans-serif';
      ctx.textAlign='center';
      ctx.fillText(
        (battle.mode==='boss'&&f.slot===0?'보스 · ':'')+f.name,
        f.x,Math.max(20,f.y-f.radius-10)
      );
      const width=Math.max(50,f.radius*2);
      const y=Math.min(699,f.y+f.radius+9);
      ctx.fillStyle='#ddd';ctx.fillRect(f.x-width/2,y,width,8);
      ctx.fillStyle='#24af42';
      ctx.fillRect(f.x-width/2,y,width*f.hp/f.maxHp,8);
      ...
      if(countdown){
        ...
        ctx.lineTo(end-9,-7);ctx.moveTo(end,0);ctx.lineTo(end-9,7);
    ...
    for(const o of battle.orbits)
      circle(o.x,o.y,8,battle.fighters[o.owner].color);
    ...
    if(countdown||battle.state==='ended'){
      ...
      ctx.fillText(countdown?String(countdown.number):battle.result,360,365);
    }
    battle.fighters.forEach((f,i)=>{
      const card=$('hp').children[i];
      if(!card)return;
      card.textContent=(f.hp<=0?'탈락 · ':'')+
  ...
  function loop(now){
    ...
    try{
      if(countdown){
        ...
        if(countdown.done){
          countdown=null;battle.start();acc=0;
        }
      }else if(battle?.state==='running'){
        acc+=dt;
        while(acc>=1/120&&battle.state==='running'){
          battle.step(1/120);acc-=1/120;
          for(const h of battle.wallHits)sound.hit(h.slot);
          for(const e of battle.audioEvents)sound.play(e.type);
        }
        if(battle.impactId>seenImpact){
          shake=.3;seenImpact=battle.impactId;
        }
        say(battle.state==='ended'?battle.result:
          Math.floor(battle.time)+'초 · 생존 '+
          battle.fighters.filter(f=>f.hp>0).length+'명');
        if(battle.state==='ended')buttons();
      }
      sound.setFlight(battle?.state==='running'&&
        battle.effects.some(e=>e.type==='moneyMissile'&&e.missiles.length));
    ...
    if(!panel.hidden)frameId=requestAnimationFrame(loop);
  }
  ...
  open.onclick=()=>{
    try{
      ...
      for(const c of config.characters){
        if(c.image){
          const img=new Image();img.src=c.image;
          images.set(c.id,img);
        }
      }
      ...
      $('mode').value='random';
      $('help').textContent='랜덤 개인전: 모두 서로 공격합니다. 크기와 체력 변경은 이번 다인전에 적용됩니다.';
      main.hidden=true;panel.hidden=false;
      preview();last=0;
      frameId=requestAnimationFrame(loop);
      panel.scrollIntoView();
    }catch(error){
      say(error.message);alert(error.message);
    }
  };

  $('add').onclick=()=>{row();preview();};
  ...
  $('start').onclick=()=>{
    ...
    tick=3;sound.play('countdownTick');buttons();
  };
  $('pause').onclick=()=>{
    if(countdown){
      countdown.paused=!countdown.paused;
      $('pause').textContent=countdown.paused?'계속하기':'일시정지';
    }else if(battle?.state==='running'){
      battle.pause();$('pause').textContent='계속하기';
    }else if(battle?.state==='paused'){
      battle.start();acc=0;sound.unlock();
      $('pause').textContent='일시정지';
    }
    sound.stopSamples();
  };
  ...
  $('back').onclick=()=>{
    ...
    sound.stopSamples();countdown=null;battle=null;
    panel.hidden=true;main.hidden=false;open.focus();
  };
